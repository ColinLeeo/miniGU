...
graph_name=aids_merged
script_dir=$(realpath "$(dirname "$0")")
log=$workspace/results/gcard/build-logs/aids_merged_k"$k".log
query_file=$(mktemp)
...
if [[ ! -f "$catalog_json" ]] || ! python3 -c "import json,sys; d=json.load(open(sys.argv[1])); sys.exit(0 if sys.argv[2] in d.get('graphs',{}) else 1)" "$catalog_json" "$graph_name"; then
  "$script_dir/build_aids_merged_graph.sh"
fi
...
run_build_with_metrics "$log" "GCard" "aids_merged" "$threads" "$db_path" \
